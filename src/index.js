...
function initRowHeader({ unit, unitHeaderHeight }) {
  ...
  let tempNumber = 1;
  let realPosition = 0;
  const w = setting.header.leftHeaderWidth;
...
function initSku({ skuList, skuAttr, setting }) {
  let rowNumber = 0;
  let columnNumber = skuList.length;
...
function initLimiteSku({ mysetting, myunit, myskuAttr }) {
  ...
  leftHeaderWidth = unit.length * setting.header.leftHeaderWidth; // 表格左侧表头宽度 --- width ---
  unitHeaderHeight = getRowHeaderUnitHeight(unit, setting.header.leftHeaderHeight); // 表格左侧表头单元格合并后每列高度 --- height ---
